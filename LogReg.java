...
public class LogReg extends Configured implements Tool {
    private static final Logger logger = LogManager.getLogger(LogReg.class);
    ...
    public static class Train_mapper extends Mapper<Object, Text, IntWritable, Text> {
        ...
        public void map(final Object key, final Text value, final Context context) throws IOException, InterruptedException {
            ...
            double h=1/(1+Math.exp(-sum));
            System.out.println(h);
            double nll=-y*Math.log(h)-(1-y)*Math.log(1-h);
            System.out.println("the nll for example:"+nll);
            double grad_term =  h-y;
            for (int i = 0; i < weight_up_sums.size(); i++) {
                if (i > 0) {
                    weight_up_sums.put(i, weight_up_sums.get(i) + Double.parseDouble(s[i]) * grad_term);
                } else {
                    weight_up_sums.put(i, weight_up_sums.get(i) + grad_term);
                }
            }
            sum_error+=nll;
            System.out.println("cummulative error:"+sum_error);
        }
    ...
    public static class Train_reducer extends Reducer<IntWritable, Text, Text,Text> {
        ...
        public void cleanup(Context context) throws IOException,InterruptedException{
            ...
            if (Math.abs(nll - error) > tolerance && nll<error) {
                for (int i=0;i<weights.size();i++) {
                    weights.put(i, weights.get(i) - lr * w_pgd.get(i)/nob);
                    if(i<weights.size()-1)
                        w+=weights.get(i)+",";
                    else
                        w+=weights.get(i);
                }
                context.getCounter(Stats.curr_error).increment((long)(nll*1000000000));
    ...
    public int run(final String[] args) throws Exception {
        while(flag==0) {
            ...
            job.setJarByClass(LogReg.class);
            ...
            sum_error=c.findCounter(Stats.curr_error).getValue()/(double)1000000000;
    ...
    public static void main(final String[] args) {
        ...
        try {
            num_it=0;
            flag=0;
            ToolRunner.run(new LogReg(), args);
